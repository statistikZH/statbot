  ...
  download.file(paste0("https://www.bfs.admin.ch/bfsstatic/dam/assets/",get_bfs_asset_nr("px-x-0102020202_102"),"/master"),destfile=destfile)
   ...
   if(check_changes_in_input_file(destfile)|flag_force_update){
     if(flag_force_update) print("flag_force_update") else print("Changes found")
      ...
      new_names <- c("citizenship_category_of_wife", "citizenship_category_of_husband", "spatialunit_name", "jahr", "value")
      ...
      df<-add_granularity_levels_up(df,list_ontologies=c("A.ADM2","A.ADM1","CH"),list_dimensions=c("citizenship_category_of_wife","citizenship_category_of_husband"))

      df<-df %>% select(all_of(GLOBAL_TOTAL_LIST), citizenship_category_of_wife, citizenship_category_of_husband)



      write.csv(df,"data/values/1_01_008_CH.csv",row.names = F)
