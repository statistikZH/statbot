#' Function do download the dataset with the id: 1_01_023_CH
#'
...
#'
#' @export
statbot_src_1_01_023_CH <- function(flag_force_update=FALSE){
  destfile<-"temp/1_01_023_CH.px"
  ...
  if(check_changes_in_input_file(destfile)|flag_force_update){
    ...
    df <- df %>% dplyr::filter(Migrationstyp=="Interkantonaler Wegzug")
    ...
    write.csv(df,"data/values/1_01_023_CH.csv",row.names = F)
