...
questions_and_queries <- tibble(
                          questions = c("how high is the {indicator} in {municipality}?", 
                           ...
                           queries=c("select value from indicator_values2 iv
                                      left join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      ...
                                      left join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      where iv.indicator_id={indicator_id} and iv.value=max(value) and i.year=max(year)",
                                     
                                      "select s.name from indicator_values2 iv
                                      left join spatialunit s on t1.spatialunit_id = s.spatialunit_id
                                      where iv.indicator_id={indicator_id} and iv.value=min(value) and t1.year=max(year)",
                                     
                                      "select max(value), min(value), avg(value) from indicator_values2 
                                      where indicator_id={indicator_id} and year=max(year)",
                                     
                                      "select count(*) from indicator_values2 iv
                                      left join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      where iv.indicator_id={indicator_id} and iv.VALUE> {random_value}
                                      and s.type_id = 1 and iv.year=max(year)",
                                     
                                      "select value from indicator_values2 
                                      where indicator_id={indicator_id} and unit_type=8 and year=max(year)",
                                     
                                      "select s.unit_name from indicator_values2 iv
                                      left join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                     where iv.value=min(value) and s.unit_type=4 and iv.indicator_id={indicator_id} and t1.year={indicator_year}"
                                      )
