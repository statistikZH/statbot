...
questions_and_queries <- tibble(
                          questions = c("how high is the {indicator} in {municipality}?", 
                           ...
                           queries=c("select value from indicator_values2 
                                      where indicator_id = {indicator_id} and spatialunit_id={municipality_id} 
                                      and year=max(year)", 
                                     
                                      "select t2.name from indicator_values2 t1
                                      left join spatialunit t2 on t1.spatialunit_id = t2.spatialunit_id
                                      where t1.indicator_id={indicator_id} and t1.value=max(value) and t1.year=max(year)",
                                     
                                      "select t2.name from indicator_values2 t1
                                      left join spatialunit t2 on t1.spatialunit_id = t2.spatialunit_id
                                      where t1.indicator_id={indicator_id} and t1.value=min(value) and t1.year=max(year)",
                                     
                                      "select max(value), min(value), avg(value) from indicator_values2 
                                      where indicator_id={indicator_id} and year=max(year)",
                                     
                                      "select count(*) from indicator_values2 t1
                                      left join spatialunit t2 on t1.spatialunit_id = t2.spatialunit_id
                                      where t1.indicator_id={indicator_id} and t1.VALUE> {random_value}
                                      ...
                                      "select t2.unit_name from indicator_values2 t1
                                      left join spatialunit t2 on t1.spatialunit_id = t2.spatialunit_id
                                     where value=min(value) and t2.unit_type=4 and indicator_id={indicator_id} and t1.year={indicator_year}"
                                      )
