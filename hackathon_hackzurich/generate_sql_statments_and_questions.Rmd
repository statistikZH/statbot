...
questions_and_queries <- tibble(
                          questions = c("how high is the {indicator} in {municipality}?", 
                           ...
                           queries=c("select value from indicator_values2 iv
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      where iv.indicator_id = {indicator_id} and s.spatialunit_id={municipality_id} 
                                      and year=max(year) and i.name like '{indicator_name}' and s.unit_name like '{municipality}'", 
                                     
                                      "select s.name from indicator_values2 iv
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      where iv.indicator_id={indicator_id} and iv.value=max(value) and i.year=max(year) and i.name like '{indicator_name}'",
                                     
                                      "select s.name from indicator_values2 iv
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      join spatialunit s on t1.spatialunit_id = s.spatialunit_id
                                      where iv.indicator_id={indicator_id} and iv.value=min(value) and t1.year=max(year) and i.name like '{indicator_name}'",
                                     
                                      "select max(value), min(value), avg(value) from indicator_values2 
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      where indicator_id={indicator_id} and year=max(year) and i.name like '{indicator_name}'",
                                     
                                      "select count(*) from indicator_values2 iv
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      where iv.indicator_id={indicator_id} and iv.VALUE> {random_value}
                                      and s.type_id = 1 and iv.year=max(year) and i.name like '{indicator_name}'",
                                     
                                      "select value from indicator_values2 
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      where indicator_id={indicator_id} and unit_type=8 and year=max(year) and i.name like '{indicator_name}'",
                                     
                                      "select s.unit_name from indicator_values2 iv
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                     where iv.value=min(value) and s.unit_type=4 and iv.indicator_id={indicator_id} and t1.year={indicator_year} and i.name like '{indicator_name}'"
                                      )
