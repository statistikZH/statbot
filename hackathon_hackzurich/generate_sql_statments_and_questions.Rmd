...
questions_and_queries <- tibble(
                          questions = c("how high is the {indicator} in {municipality}?", 
                                       ...
                                       "Which municipality has the minimum {indicator}?",
                                        ...
                                        "How many municipalities currently have a {indicator} higher than {random_value}?",
                           ...
                           queries=c("select iv.value from indicator_values2 iv
                                      ...
                                      where i.name like '{indicator_name}' and s.unit_name like '{municipality}'", 
                                      ...
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      where iv.VALUE> {random_value} and s.type_id = 1 and iv.year=max(year) and i.name like '{indicator_name}'",
                                     
                                      "select iv.value from indicator_values2 iv
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      where unit_type=8 and year=max(year) and i.name like '{indicator_name}'",
                                     
                                      "select s.unit_name from indicator_values2 iv
                                      join indicators i on iv.indicator_id = i.indicator_id
                                      join spatialunit s on iv.spatialunit_id = s.spatialunit_id
                                      where iv.value=min(value) and s.unit_type=4 and t1.year={indicator_year} and i.name like '{indicator_name}'"
                                      )
