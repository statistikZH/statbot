...
```{sql connection=statext, output.var = "indikator"}
select * from GPZH_TEST.T_INDIKATOR a
join (select * from GPZH_TEST.T_RELABS) b
on a.RELABS_ID = b.RELABS_ID where a.IS_ONLINE = 1
-- RELABS BEINHALTET Beschreibung zu prozentual, absolut etc.
```

```{sql connection=statext, output.var = "raum"}
select * from GPZH_TEST.T_RAUMEINHEIT a
join (select * from GPZH_TEST.T_RAUMEINHEIT_TYP) b
on a.TYP_ID = b.RAUMEINHEIT_TYP_ID where b.RAUMEINHEIT_TYP_ID != 9
-- RAUMEINHEIT_TYP_ID 9 means inactive and we do not want this
```
```{sql connection=statext, output.var = "indikatorvalue"}
select * from GPZH_TEST.T_INDIKATOR_VALUE
```
```{sql connection=statext, output.var = "relabs"}
select * from GPZH_TEST.T_RELABS
```
...
Next steps: 
- The Umlaut have to be deleted everywhere, or they have to be activated on the server
- Define the measures
