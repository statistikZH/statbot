# Script to create a dimension table for the dataset:  1_01_028
#
...
destfile<-"temp/1_01_028.px"

if(!file.exists(destfile)){
  download.file(paste0("https://www.bfs.admin.ch/bfsstatic/dam/assets/",get_bfs_asset_nr("px-x-0103030000_299"),"/master"),destfile=destfile)
}
...
unique_dimension_names <- c("domicile_1_year_ago", "citizenship_selection", "age_group_20_pension_15")

# should any dimension be ignored? the reason could be, that it already exists
ignore_dimensions <- c("Bevölkerungstyp", "Geschlecht")
